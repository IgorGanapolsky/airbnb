...
class ContentGenerator:
    def __init__(self):
        ...
        self.affiliate_link = self.config['affiliate']['airbnb_link']
    ...
    def _add_affiliate_links(self, content: str, subtle: bool = False) -> str:
        if not self.affiliate_link or self.affiliate_link == "YOUR_AIRBNB_AFFILIATE_LINK":
            return content

        short_link = self._create_short_link(self.affiliate_link)

        if subtle:
            content += f"\n\n{self.disclosure}\nFind great Airbnb deals: {short_link}"
        else:
            content = content.replace(
                "Airbnb",
                f"[Airbnb]({short_link})"
            )
            content += f"\n\n---\n*{self.disclosure}*\n\n**[🏡 Find Amazing Airbnb Deals Here]({short_link})**"
